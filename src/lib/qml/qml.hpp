...
class SceneGraph : public QObject
{
    ...
    public:
        ...
        Q_INVOKABLE int run_command_string(QString str) { status p = qml::command::run_command_string(str.toStdString()); emit commandMessageChanged(p.msg); return p.state; };

    signals:
        void selectionChanged(int type, int uid, int nid, int fid);
        void nodeSelection(int type, int uid, int nid);
        void commandMessageChanged(std::string msg);
};
