...
void Pipelines::bindMesh(VkDevice device, VkCommandBuffer buffer, Node* node, VkDeviceSize offsets[1])
{
    ...
    // Shading
    vkCmdBindPipeline(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_meshPipelines.wire);
    ...
    // Shading 
    vkCmdBindPipeline(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_meshPipelines.point);
...
VkPipeline Pipelines::pipeline(Node::Type node)
{
    return m_meshPipelines.solid;
}
...
void Pipelines::prepare(VkDevice device, VkRenderPass renderPass, VkPipelineVertexInputStateCreateInfo* vi)
{
    ...
    // Wireframe shader
    shaderStages[0] = loadShader(device, "shaders/spv/edge.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(device, "shaders/spv/edge.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
    shaderStages[2] = loadShader(device, "shaders/spv/edge.geom.spv", VK_SHADER_STAGE_GEOMETRY_BIT);
    ...
    shaderStages[1] = loadShader(device, "shaders/spv/point.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
    shaderStages[2] = loadShader(device, "shaders/spv/point.geom.spv", VK_SHADER_STAGE_GEOMETRY_BIT);

    // Point rendering pipeline 
    err = vkCreateGraphicsPipelines(device, m_pipelineCache, 1, &pipelineEdgeCreateInfo, nullptr, &m_meshPipelines.point);
    assert(!err);

    // Base solid shader
    shaderStages[0] = loadShader(device, "shaders/spv/base.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(device, "shaders/spv/base.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
